...
2. **Anything that widens what the automation may do.** A `permissions:` block, a new secret reference, a trigger that runs against a fork's code, a third-party action, an action pinned to a tag that can move.
